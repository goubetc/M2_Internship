function u   = ARTReconstructionRd(A,d,relaxParam,numIterART,u,theta)
% Modification !!! of u   = ARTReconstruction(A,d,relaxParam,numIterART,u)
%
...
% Norm of rows
% Anorm               = sum(A.*A,2);
Anorm           = sum(ones(size(u)));

% Loop across iterations
for it = 1:numIterART
    % Loop across rows
    for ir = 1:length(theta)
        thetaThis   = theta(ir);
        num         = d(ir)-A(u);
