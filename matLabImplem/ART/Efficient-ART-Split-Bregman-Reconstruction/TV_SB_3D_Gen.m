...
%         [x,y,z]       = shrink3(dx+bx,dy+by,dz+bz,alpha/lambda);
        ...
        z           = shrink1(dz+bz,7*(alpha(1))/lambda);
    ...
    if (outer == 230)||(outer == 250)||(outer == 260)
        %break
        outer;
    end
...
%         end
        ...
        %%% Added new error evaluations
        for i = 1:size(uTarget,1)
            % Pick Signal To Noise Ratio
            ePsnr(it) = ePsnr(it) + psnr(squeeze(uTarget(i,:,:)), squeeze(u(i,:,:)));
            % TV
            sumDiff = squeeze(uTarget(i,:,:)) - squeeze(u(i,:,:));
            [gDiff_x, gDiff_y] = gradient(sumDiff);
            tv(it) = tv(it) + sum(sum(sqrt(gDiff_x.^2 + gDiff_y.^2)));
        end
        ePsnr(it)     = ePsnr(it)/size(uTarget,1);
        tv(it)        = tv(it)/size(uTarget,1);
